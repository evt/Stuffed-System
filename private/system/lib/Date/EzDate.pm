...
# version modified by the Eych
$VERSION = '1.081';

# modification history
# May 07, 2008 - added 3 subs (date_range_string, time_range_string and day_lumps) from version 1.10
# Dec 27, 2003 - put one of timelocal calls in eval to catch its internal errors

=head1 MODIFICATIONS by Stuffed Guysd
...
comment: I was not able to assign a negative value to the 'hour' property
because it was passed directly to timelocal() which doesn't except such 
things. I've change this part from using timelocal to how other properties are
changed and now it seems to be working correctly
...
sub timefromfull {
        ...
        return eval {timelocal($sec, $min, $hour, $day, $month, $year)};
...
I made a non-backwards compatible change to "overload" because the same variable is now being used for 
non-comparison overloads like addition and subtraction.
