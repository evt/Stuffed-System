...
# Modified in 09.2004 by the Eych
# = added 'delimiter' parameter to new method to specify alternative delimiter
# Modified in 09.2002 by the Eych
# - now allows all characters in the string
...
#
# Version 0.012   09/2004 (Sergey Smirnov)
#    added 'delimiter' parameter to new method to specify alternative delimiter
# Version 0.011   09/2002 (Sergey Smirnov)
#    now allows any characters in the string
...
sub new {
  ...
  my $self = {};
  my $in = {
    delimiter => undef, # optional, any string that will be used as a delimiter
    @_
  };
  $self->{'_STATUS'} = undef;
  ...
  my $delim = ',';
  $delim = $in->{delimiter} if defined $in->{delimiter} and $in->{delimiter} ne '';
...
#      }
  ...
  $combination .= $self->{_DELIMITER};
      ...
      # putting quotes in the beginning and the end of the field if a coma
      # or quotes are spotted inside the string
      my $quotes = $column =~ /[$self->{_DELIMITER}\042]/;
      $combination .= "\042" if $quotes;
      $combination .= "$column";
      $combination .= "\042" if $quotes;
    }
...
sub _bite {
  ...
  my $dmtr = quotemeta($self->{_DELIMITER});
